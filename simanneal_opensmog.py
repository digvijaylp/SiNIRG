...
"""
Script for performing 5 cycles of simulated annealing 
to generate structures with different levels of folded-ness.
...
parser.add_argument("-c","-cmap",type=str,help="Input SuBMIT-contact-map file.")
parser.add_argument("-n","-nrep",type=int,help=
"Number of non-interacting replicas. Default 1",default=1)
parser.add_argument("-steps",type=int,
help="Number of simulations steps. Default=10000000",default=10000000)
args=parser.parse_args()

def simAnnealing(n_rep):
    # run simulated annealing to generate non-interacting replicas
    ...
    assert args.c is not None, "Please provide c-map file using -c option."
    sbm_1_grofile=args.g      # GRO file
    sbm_1_topfile=args.p      # TOP file
    sbm_1_xmlfile=args.x      # XML file
    sbm_1_cmapfile=args.c     # c-map file
    ...
    n_cycles=5  
    # 1 cycle: temperature schedule and corresponding
    ...
    for i in range(n_cycles):
        ...
        for j in range(len(temperatures)):
            ...
            sbm_1.simulation.context.setVelocitiesToTemperature\
(T_in_K*unit.kelvin)
    ...
    # load native contact pairs and distances (convert to nm)
    ...
    # Use Q values to add indices of structures to their Q0 bin.
    str_indices=[]
    for q in Q0:
    ...
    # combining single-copy mdtraj topology object to n_rep object
    ...
    for i in range(n_rep):
        ...
        for j in range(n_rnd_str):
            ...
            # Move replica coordinates centroid to origin [0,0,0]
            ...
            rep_trj.save_gro("SimAn_00/rep%d_%d.gro"%(i,j))
            print ("Generate SimAn_00/rep%d_%d.gro"%(i,j))
    ...
    for j in range(n_rnd_str):
        ...
        combined_xyz[j]=\
            np.concatenate([combined_xyz[j][i] for i in range(n_rep)])

        # Creating n_rep mdtraj trajectory object using combined topology
        combined_trj=md.Trajectory(xyz=combined_xyz[j],
                                   topology=combined_topol)
        ...
        combined_grofile=\
                sbm_1_grofile.replace(".gro",".%dnir_%d.gro"%(n_rep,j))
...
print ("> Generating NIR structure file with different Q values.")
n_grofile=simAnnealing(n_rep=args.n)
